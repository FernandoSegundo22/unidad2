...
export const isAdmin = async (req,res,next) => {
    ...
    //Buscar los roles del ususario
    const roles = await Role.find({ _id: {$in: user.roles} });
    //console.log(roles);

    //Recorrer roles de usuario
    ...
    return res.status(403).json({message: "Error: Solo un administrador puede agregar"});
}
...
for(let i=0; i < roles.length; i ++){
    ...
    return res.status(403).json({message: "Error: Solo un administrador o moderador puede modificar"});
